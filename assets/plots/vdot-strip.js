(function () {
	...
	function _templateObject$2() {
	  var data = _taggedTemplateLiteral(["# Vertical dot strip with regression band"]);
	...
	function define$2(runtime, observer) {
	  ...
	  main.variable(observer("make")).define("make", ["setDefaults", "config", "defaults", "d3", "getDimensions", "flatten", "makeScaleX", "makeScaleY", "makeRegression", "makeAxisX", "makeAxisY", "plotPoints", "makeRegressionBand"], function (setDefaults, config, defaults, d3, getDimensions, flatten, makeScaleX, makeScaleY, makeRegression, makeAxisX, makeAxisY, plotPoints, makeRegressionBand) {
	    return function () {
	      ...
	      var categories = config.data.map(function (d) {
	        return d.key;
	      });

	      var _getDimensions = getDimensions(categories),
	      ...
	      var scaleX = makeScaleX(categories, w - 60),
	          scaleY = makeScaleY(flat.map(function (d) {
	        return d.y;
	      }), h),
	          regression = makeRegression(flat, scaleX, scaleY);
	      var svg = d3.create("svg").attr("viewBox", [0, 0, fw, fh]).attr("color", "rgba(5, 5, 38, 0.5)");
	      var axisX = svg.append("g").attr("transform", "translate(".concat(margin.left + 30, ",").concat(fh - margin.bottom + 10, ")"));
	      ...
	      var pointsLayer = svg.append("g").attr("transform", "translate(".concat(margin.left + 30, ",").concat(margin.top, ")"));
	      plotPoints(pointsLayer, flat, scaleX, scaleY, config.colorPoints);
	      var regressionLayer = svg.append("g").attr("transform", "translate(".concat(margin.left + 30, ",").concat(margin.top, ")"));
	  ...
	  main.variable(observer("plotPoints")).define("plotPoints", function () {
	    return function (g, points, scaleX, scaleY, colorPoints) {
	      g.attr("fill", "none").attr("pointer-events", "all").selectAll("circle").data(points).join("circle").attr("r", 4.5).attr("stroke", "none").attr("fill", colorPoints).attr("cx", function (d) {
	        return scaleX(d.x);
	      }).attr("cy", function (d) {
	        return scaleY(d.y);
	      });
	  ...
	  main.variable(observer("makeRegressionBand")).define("makeRegressionBand", function () {
	    return function (g, regression, colorRegression) {
	      g.append("path").attr("fill", "none").attr("pointer-events", "all").attr("stroke", colorRegression).attr("stroke-width", 30).datum(regression.points).attr("d", regression.lineGenerator);
	    };
	  });
	  main.variable(observer("makeAxisY")).define("makeAxisY", ["d3"], function (d3) {
	    return function (g, scaleY, w) {
	  ...
	  main.variable(observer("makeScaleY")).define("makeScaleY", ["d3"], function (d3) {
	    return function (values, h) {
	      var extent = d3.extent(values);
	      return d3.scaleLinear().domain([extent[1], extent[0]]).nice().range([0, h]);
	    };
	  });
	  main.variable(observer("makeScaleX")).define("makeScaleX", ["d3"], function (d3) {
	    return function (categories, w) {
	      return d3.scalePoint().domain(categories).range([0, w]);
	    };
	  });
	  ...
	  main.variable(observer("flatten")).define("flatten", function () {
